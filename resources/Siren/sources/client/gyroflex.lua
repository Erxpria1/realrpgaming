...
local strobo_interval = 5
local is_strobo_enabled = enable
local flash_interval = 2
...
function checkForAbility(vehicle)
	...
	if (config.car_lights_table[veh_model]==nil) or (getElementData(vehicle, "emerlights_enabled") == "false") then
		return false
	end
	return true
end
...
function strobo_state_update (vehicle)
	...
	if (isElement(vehicle)) then
		if (vehicles[vehicle]==nil) then
			...
			--if (config.car_lights_table[veh_model]==nil) then
			--veh_model = "default"
			--end
			
			local occupant = getVehicleOccupant( vehicle, 0)
			if (occupant and isElement(occupant) and getElementType(occupant) == "player") then	-- peds also can use emerlights
			...
			local mode = config.car_lights_table[veh_model]["mode"]
			if (mode==nil) then
				mode = "lamp"
			end
			...
			for light_id, light_desc in pairs(config.car_lights_table[veh_model]) do
