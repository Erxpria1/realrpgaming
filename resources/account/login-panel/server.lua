			...
			if result and client then
				if #result > 0 then
					...
					if tonumber(accountData["activated"]) == 0 then
						-- Auto activate if 0 for this private server
						dbExec(exports.mysql:getConn("core"), "UPDATE `accounts` SET `activated`='1' WHERE id=?", accountData["id"])
						accountData["activated"] = "1"
					end
...
function goFromLoginToSelectionScreen(player)
	...
	-- Bypass application system for instant play
	if (getElementData(player, "appstate") or 0) < 3 then
		...
		setElementData(player, "appstate", 3)
	end
