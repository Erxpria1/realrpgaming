...
function displayMechanicJob()
	outputChatBox("#FF9933Hizmet vermek için #FF0000sağ tık menüsünü#FF9933 kullanın.", 255, 194, 15, true)
end
			...
			if not (wMechanic) then
				...
				if not isPedInVehicle( localPlayer ) and getPedSimplestTask(localPlayer) ~= "TASK_SIMPLE_CAR_DRIVE" then
					...
					if getElementHealth(vehicle) > 950 then
						bMechanicTwo = guiCreateButton( 0.05, y, 0.9, 0.1, "Hızlı Tamir - $110", true, wMechanic )
					...
					if vehicleWithPaintjob[getElementModel(vehicle)] then
						bMechanicSix = guiCreateButton( 0.05, y, 0.9, 0.1, "Özel Boya - $7.500", true, wMechanic )
					...
					-- Replace lights
					--if vehicleWithPaintjob[getElementModel(vehicle)] then
						bMechanicNine = guiCreateButton( 0.05, y, 0.9, 0.1, "Far Değişimi - $1.250", true, wMechanic )
					...
					if getVehicleUpgradeOnSlot(vehicle, 8) ~= 0 and faction[4] then
						bMechanicSeven = guiCreateButton( 0.05, y, 0.9, 0.1, "NOS Kaldır", true, wMechanic )
						addEventHandler( "onClientGUIClick", bMechanicSeven, removeNosFromVehicle, false)
						y = y + 0.1
					end
				end

				-- Cam Filmi Ekle/Kaldır
				bMechanicEight = guiCreateButton( 0.05, y, 0.9, 0.1, "Cam Filmi", true, wMechanic )
				y = y + 0.1
...
function tyreWindow()
	...
	if not (wTyre) then
		-- Create the window
		wTyre = guiCreateWindow(X+100, Y, Width, Height, "Değiştirilecek lastiği seçin.", false )
...
function paintWindow()
	...
	if not (wPaint) then
		wPaint = guiCreateWindow(windowX, windowY, windowWidth, windowHeight, "Araç Boyama", false )
		bPaintColor1 = guiCreateButton( 0.05, 0.1, 0.45, 0.2, "Renk 1 Seç", true, wPaint )
		addEventHandler( "onClientGUIClick", bPaintColor1,  function(button, state)
			if(button == "left" and state == "up") then
				paintColor( 1 )
			end
		end, false)
		bPaintColor2 = guiCreateButton( 0.55, 0.1, 0.45, 0.2, "Renk 2 Seç", true, wPaint )
		addEventHandler( "onClientGUIClick", bPaintColor2,  function(button, state)
			if(button == "left" and state == "up") then
				paintColor( 2 )
			end
		end, false)
		bPaintColor3 = guiCreateButton( 0.05, 0.4, 0.45, 0.2, "Renk 3 Seç", true, wPaint )
		addEventHandler( "onClientGUIClick", bPaintColor3,  function(button, state)
			if(button == "left" and state == "up") then
				paintColor( 3 )
			end
		end, false)
		bPaintColor4 = guiCreateButton( 0.55, 0.4, 0.45, 0.2, "Renk 4 Seç", true, wPaint )
...
function paintjobWindow()
	...
	if not (wPaintjob) then
		...
		-- Create the window
		wPaintjob = guiCreateWindow(X+100, Y, Width, Height, "Yeni bir Özel Boya seçin.", false )

		-- Özel Boya 1
		bPaintjob1 = guiCreateButton( 0.05, 0.1, 0.9, 0.17, "Özel Boya 1", true, wPaintjob )
		...
		-- Özel Boya 2
		bPaintjob2 = guiCreateButton( 0.05, 0.3, 0.9, 0.17, "Özel Boya 2", true, wPaintjob )
		...
		-- Özel Boya 3
		bPaintjob3 = guiCreateButton( 0.05, 0.5, 0.9, 0.17, "Özel Boya 3", true, wPaintjob )
		...
		-- Özel Boya 4
		bPaintjob4 = guiCreateButton( 0.05, 0.7, 0.9, 0.17, "Özel Boya 4", true, wPaintjob )
