...
function quitJob(job)
	if (job==1) then -- TRUCKER JOB
		exports["job-system-trucker"]:resetTruckerJob()
		outputChatBox("Teslimat şoförü işinizden ayrıldınız.", 0, 255, 0)
	elseif (job==2) then -- TAXI JOB
		resetTaxiJob()
		outputChatBox("Taksi şoförü işinizden ayrıldınız.", 0, 255, 0)
	elseif (job==3) then -- BUS JOB
		resetBusJob()
		outputChatBox("Otobüs şoförü işinizden ayrıldınız.", 0, 255, 0)
	elseif (job==4) then -- CITY MAINTENANCE
		outputChatBox("Şehir bakım görevlisi işinizden ayrıldınız.", 0, 255, 0)
		triggerServerEvent("cancelCityMaintenance", localPlayer)
	elseif (job==5) then -- MECHANIC
		outputChatBox("Tamirci işinizden ayrıldınız.", 0, 255, 0)
	elseif (job==6) then -- LOCKSMITH
		outputChatBox("Çilingir işinizden ayrıldınız.", 0, 255, 0)
	end
end
