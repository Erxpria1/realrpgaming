...
setElementData(fishermanJohn, "name", "Balıkçı John")
...
function startFishingSession()
    if exports['item-system']:hasItem(localPlayer, 49, 1) then
        if getElementData(localPlayer, "isfishing") then
            outputChatBox("Zaten balık tutuyorsunuz...", 255, 0, 0)
        else 
            if isPedInVehicle(localPlayer) then
                return outputChatBox("Tekne kullanırken balık tutamazsınız.", 255, 0, 0) 
            end
            if onBoat() then
                triggerServerEvent("sendAmeClient", localPlayer, "olta atıyor.")
                ...
                mgTimer = setTimer(
                    function()
                        if not onBoat() then
                            endFishingSession()
                            return outputChatBox("Suda kalmayı başaramadınız.", 255, 0, 0)
                        end
                        pFish = guiCreateProgressBar(0.425, 0.75, 0.2, 0.035, true)
                        exports.hud:sendBottomNotification(localPlayer, "Balıkçılık", "Bir şey kaptınız! Oltayı çekmek için [ ve ] tuşlarını kullanın.")
                        bindKey("[", "down", beginFishingGame)
                        -- Balığın kaçması için zamanlayıcı başlat
                        escapeTimer = setTimer(
                            function() 
                                endFishingSession()
                                outputChatBox("Oltanızı çekerken koptuğunu fark ettiniz.", 255, 0, 0)
                                triggerServerEvent("fishing:takeRod", localPlayer, localPlayer)
                            end, 
                        math.random(8000, 10000), 1)
                    end
                , math.random(180000, 600000), 1)  
                outputChatBox("/stopfishing yazarak istediğiniz zaman balık tutmayı bırakabilirsiniz.", 0, 255, 0)
            else 
                outputChatBox("Buradan balık tutamazsınız.", 255, 0, 0)
            end
        end
    else
        outputChatBox("Olta çubuğunuz yok.", 255, 0, 0)
    end
end
...
function endFishingSession()
    if (getElementData(localPlayer, "isfishing")) then
        ...
        triggerServerEvent("sendAmeClient", localPlayer, "oltasını çeker.")
...
function sellFish(fish, price)
    ...
    sellfishGUI = guiCreateWindow(744, 301, 303, 117, "Balık Sat", false)
    exports.global:centerWindow(sellfishGUI)
    showCursor(true)
    guiWindowSetSizable(sellfishGUI, false)

    sellfishLabel = guiCreateLabel(4, 18, 295, 44, fish .. " balığınız var ve $" .. price .. " kazanacaksınız.", false, sellfishGUI)
    guiLabelSetHorizontalAlign(sellfishLabel, "center", true)
    guiLabelSetVerticalAlign(sellfishLabel, "center")
    sellfishbutton = guiCreateButton(10, 68, 140, 34, "Balıkları Sat", false, sellfishGUI)
    cancelfishbutton = guiCreateButton(153, 68, 140, 34, "İptal", false, sellfishGUI) 
